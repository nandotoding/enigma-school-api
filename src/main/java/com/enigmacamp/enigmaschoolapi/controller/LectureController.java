...
public class LectureController {
    ...
    public ResponseEntity listAllLectures(
        ...
        Page<Lecture> lectures = lectureService.list(page, size, dir, sortBy);

        if (lectures.isEmpty()) {
            throw new NotFoundException();
        }

        return ResponseEntity.status(HttpStatus.OK).body(new PagingResponse<>("Get lecturers successfully", lectures));
    }
}
